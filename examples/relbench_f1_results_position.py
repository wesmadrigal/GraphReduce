#!/usr/bin/env python
"""RelBench rel-f1: results position autocomplete example aligned to the official task definition."""
...
def run_rel_f1_results_position(
    ...
    try:
        ...
        for node in [result_node, driver_node, constructor_node, race_node, circuit_node]:
            graph.add_node(node)

        graph.add_entity_edge(result_node, driver_node, parent_key=result_driver_col, relation_key=driver_id_col, reduce=False)
    ...
    df_train = all_rows[all_rows["res_date"] <= train_end].copy()
    df_val = all_rows[(all_rows["res_date"] > VALIDATION_CUT_DATE) & (all_rows["res_date"] <= val_end)].copy()
    df_test = all_rows[all_rows["res_date"] > TEST_CUT_DATE].copy()
    ...
    feature_columns = [
        ...
        and column not in REMOVED_COLUMNS
        and "label" not in column.lower()
        and not column.lower().endswith("_id")
        and column not in {"res_resultId", "res_raceId", "res_driverId", "res_constructorId"}
...
def main() -> None:
    df_train, df_val, df_test, val_metrics, test_metrics, n_features, materialized, target = run_rel_f1_results_position()

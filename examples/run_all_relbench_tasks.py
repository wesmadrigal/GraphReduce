...
def discover_tasks(match_filters: list[str], explicit_tasks: list[str]) -> list[Path]:
    primary_tasks = {
        path
...
def run_task(task_path: Path, python_executable: str, output_dir: Path, stream_output: bool) -> TaskResult:
    ...
    fields, highlights, skip_reason, error_excerpt = parse_output(stdout, stderr)
...
def main() -> int:
    ...
    print(f"Running {len(tasks)} relbench task(s) from {REPO_ROOT} using {args.python}", flush=True)

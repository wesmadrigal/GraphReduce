...
class PlowKeyException(Exception): pass
...
def plowchecks(func):
    def wrapped(*args, **kwargs):
        func(*args, **kwargs)
        # check that they implemented
        # the appropriate attributes
        attrs = ['prefix', 'env', 'db', 'schema', 'table',
                'pk', 'date_key', 'pk_child_name',
                'datatype']
        if args[0].__class__.__name__ != "Plow":
            for k in attrs:
                if not getattr(args[0], k):
                    raise Exception(f"Instances of type: {args[0].__class__.__name__} needs attribute: {k}")
    return wrapped

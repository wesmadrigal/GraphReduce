...
# Build GraphReduce dataframe for user-clicks
# (parent: UserInfo, label node: VisitsStream, click annotation from SearchInfo)
df, auc, n_features, downloaded, target = run_avito_task("user_clicks", data_dir=out_dir)
...
numeric_cols = [c for c in df.select_dtypes(include=[np.number]).columns if c != "user_clicked_next_4d"]
feature_cols = [
    c
...
* `examples/relbench_avito_user_clicks.py`
* `examples/relbench_avito_user_clicks_local_runner.py`

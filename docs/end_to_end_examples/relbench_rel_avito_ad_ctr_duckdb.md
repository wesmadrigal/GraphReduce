...
This example uses `AdsInfo.parquet` as the parent node and predicts ad-level CTR in the next 4-day horizon (implemented as `label_period_days=5` because the label window end is exclusive).

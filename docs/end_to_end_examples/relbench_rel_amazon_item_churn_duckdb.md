...
[![RelBench rel-amazon item-churn graphreduce flow](relbench_rel_amazon_item_churn_overview.svg)](relbench_rel_amazon_item_churn_overview.svg)

Open full-size: [SVG](relbench_rel_amazon_item_churn_overview.svg)

This example implements the RelBench rel-amazon item churn setup:
...
* target: whether product gets any review in next 3 months
...
df, auc, n_features, downloaded, target = run_amazon_task("item_churn", data_dir=out_dir)
...
numeric_cols = [c for c in df.select_dtypes(include=[np.number]).columns if c != "item_has_review_next_90d"]
feature_cols = [
    c
...
* `examples/relbench_amazon_item_churn.py`

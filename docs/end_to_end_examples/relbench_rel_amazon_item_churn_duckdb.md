...
[![rel-amazon item churn graphreduce flow](relbench_rel_amazon_item_churn_overview.svg)](relbench_rel_amazon_item_churn_overview.svg)

Open full-size: [SVG](relbench_rel_amazon_item_churn_overview.svg)

This page is intentionally explicit and sequential (no shared helper module imports).

**Flow**
1. Download rel-amazon parquet tables
2. Create DuckDB views and infer key/date/amount columns
3. Define GraphReduce nodes and entity edges
4. Run GraphReduce SQL transformations to build the feature frame
5. Build target label column from forward-window labels
6. Train CatBoost and print ROC AUC

<details>
<summary>Show Full Runner Script</summary>
...
def main() -> None:
    ...
    for c in label_cols:
        df[c] = df[c].fillna(0)
    df["item_has_review_next_90d"] = (df[label_cols].sum(axis=1) > 0).astype("int8")
...
* `examples/relbench_amazon_item_churn_local_runner.py`

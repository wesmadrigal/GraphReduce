...
def build_item_ltv_frame(data_dir: Path, cut_date: datetime.datetime):
    con = duckdb.connect()
    try:
        ...
        gr = GraphReduce(
            name=f"rel_amazon_item_ltv_{cut_date.date()}",
...
df_eval = build_item_ltv_frame(data_dir, VALIDATION_CUT_DATE)
df_holdout = build_item_ltv_frame(data_dir, HOLDOUT_CUT_DATE)

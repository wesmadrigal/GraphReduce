...
2. Build an eval frame at `2015-01-01`
3. Build a holdout frame at `2016-01-01`
4. For each frame: infer columns, create views, define GraphReduce graph, run SQL transforms
5. Build `item_ltv_90d_usd` from forward-window labels
6. Train CatBoostRegressor on eval and score holdout MAE
...
```python
#!/usr/bin/env python
"""Run rel-amazon item-ltv example for docs interactive mode."""
...
def main() -> None:
    print("Running rel-amazon item-ltv example...", flush=True)
    ...
    for split_name, cut_date in [("eval", VALIDATION_CUT_DATE), ("holdout", HOLDOUT_CUT_DATE)]:
        ...
        try:
            ...
            gr = GraphReduce(
                name=f"rel_amazon_item_ltv_{split_name}",
        ...
        for c in label_cols:
            df[c] = df[c].fillna(0)
        df["item_ltv_90d_usd"] = df[label_cols].sum(axis=1).astype("float64")
